...
```sh
# 1. export the worlds (once per generator version; deterministic from --seed)
cargo run --release -p knowledgedrift -- --sizes 100,500,1500 --export eval/data/knowledgedrift

# 2. replay through an adapter (each has its own venv — see its section)
eval/knowledgedrift/adapters/.venv-langmem/bin/python3 eval/knowledgedrift/adapters/run.py \
    --adapter langmem --script eval/data/knowledgedrift/knowledgedrift-500-seed1.json \
    --out eval/knowledgedrift/adapters/out/langmem-500.json

# 3. grade
cargo run --release -p knowledgedrift -- --grade eval/knowledgedrift/adapters/out/langmem-500.json \
    --script eval/data/knowledgedrift/knowledgedrift-500-seed1.json \
    --json eval/knowledgedrift/adapters/out/langmem-500-graded.json

# 4. table rows for the README
python3 eval/knowledgedrift/adapters/table.py eval/knowledgedrift/adapters/out/langmem-500-graded.json
```
...
Every arm below embeds with the same model the product uses
(`BAAI/bge-small-en-v1.5`, via the `fastembed` package), so a difference in
the tables is a difference in mechanism, not in embedder.

...
.venv-langmem/bin/python3 run.py --adapter langmem \
    --script ../../data/knowledgedrift/knowledgedrift-100-seed1.json --out out/langmem-100.json

cd ../../..   # repo root
cargo run --release -p knowledgedrift -- --grade eval/knowledgedrift/adapters/out/langmem-100.json \
    --script eval/data/knowledgedrift/knowledgedrift-100-seed1.json \
    --json eval/knowledgedrift/adapters/out/langmem-100-graded.json
```
...
The same embedder the product uses, so a difference against the engram arm is
...
**1500 world — extrapolated** (10 816 ops: 2 528 inscribes, 150 supersedes,

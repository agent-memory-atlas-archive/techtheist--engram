...
hybrid dense + BM25 scoring, the product's embedder
...
```sh
cd eval/knowledgedrift/adapters
...
.venv-mem0/bin/python3 run.py --adapter mem0 \
    --script ../../data/knowledgedrift/knowledgedrift-100-seed1.json --out out/mem0-100.json
cd ../../.. && cargo run --release -p knowledgedrift -- \
    --grade eval/knowledgedrift/adapters/out/mem0-100.json \
    --script eval/data/knowledgedrift/knowledgedrift-100-seed1.json \
    --json eval/knowledgedrift/adapters/out/mem0-100-graded.json
```
...
6. **Embedder runtime differs from the product's.** The product embeds
...
Reading it, next to the in-process arms on the same script (README
"Results — 2026-09-13": rag 71% / 0.516 / 54, grep 62%, engram 92% / 0.947 / 601):
...
Single seed; nothing here is stable to the digit until a seed ladder says so.

## Wall-clock estimate for the larger worlds
...
- `out/mem0-100.json`, `out/mem0-100-graded.json`, `out/mem0-100-graded.log` — the 100 receipts
